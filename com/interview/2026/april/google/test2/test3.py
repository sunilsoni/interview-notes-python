def getMinTime(n, d, x, y):
    ...
    def find(i):
        # FIX: Replaced recursive find with an iterative loop.
        # This prevents "maximum recursion depth exceeded" errors on large inputs.
        while p[i] != i:
            # Path halving: keeps the tree flat to maintain speed
            p[i] = p[p[i]]
            i = p[i]
        return i
    ...
    # Group points with their original index to track them
    pts = sorted([(x[i], y[i], i) for i in range(n)])

    # Check vertical/same-X connections
    for i in range(n - 1):
        if pts[i][0] == pts[i + 1][0] and pts[i + 1][1] - pts[i][1] <= d:
            union(pts[i][2], pts[i + 1][2])

    # Check horizontal/same-Y connections
...
def test_solution():
    cases = [
        ...
        # Large scale straight-line test that causes recursion error in old code
        {"args": (100000, 1, list(range(100000)), [0] * 100000), "expected": 1}
    ]
